...
/* Add a student to the queue with student_name and a question about
 * course_code.
...
int remove_ta(Ta **ta_list_ptr, char *ta_name) {
    ...
    if (head == NULL) {
        ...
        *ta_list_ptr = head->next;
        free_student(head->current_student);
    ...
    while (head->next != NULL) {
        if (strcmp(head->next->name, ta_name) == 0) {
            ...
            free_student(ta_tofree->current_student);

            head->next = head->next->next;
            // memory for the student has been freed. Now free memory for
            // the TA.
            free(ta_tofree->name);
            free(ta_tofree);
            return 0;
 ...
 * If to_serve is NULL (i.e. the method is called with no student to take),
 * then
...
// return a message about which TAs are serving which students
char *print_currently_serving(Ta *ta_list) {
    if (ta_list == NULL) {
        //35 is length of string
        char *ret = malloc(sizeof(char) * 35);
        if (ret==NULL){
            perror("malloc");
            exit(1);
        }
        strcpy(ret, "No TAs are currently working.\r\n");
        return ret;
    }
    char *ret = malloc(sizeof(char) * OUT_BUF_SIZE);
    if (ret == NULL){
        perror("malloc");
        exit(1);
    }
    while (ta_list != NULL) {
        if (ta_list->current_student != NULL) {
            //1 is nullterm
            strncat(ret, "TA: ", OUT_BUF_SIZE - 1 - strlen(ret));
            strncat(ret, ta_list->name, OUT_BUF_SIZE - 1 - strlen(ret));
            strncat(ret, " is serving ", OUT_BUF_SIZE - 1 - strlen(ret));
            strncat(ret, ta_list->current_student->name,
                    OUT_BUF_SIZE - 1 - strlen(ret));
            strncat(ret, ".\r\n", OUT_BUF_SIZE - 1 - strlen(ret));
        } else {
            strncat(ret,"TA: ", OUT_BUF_SIZE-1-strlen(ret));
            strncat(ret, ta_list->name,OUT_BUF_SIZE-1-strlen(ret));
            //took out network newline
            strncat(ret, " has no student\r\n",OUT_BUF_SIZE-1-strlen(ret));
        }
...
/*  Return a list of all the students in the queue. This does not
...
char *print_full_queue(Student *stu_list) {
    ...
    strncpy(ret, "Full_Queue\r\n", OUT_BUF_SIZE);
    //1 is null term
    for (; stu_list != NULL; stu_list = stu_list->next_overall) {
        strncat(ret, "Student ", OUT_BUF_SIZE - 1 - strlen(ret));
        strncat(ret, stu_list->name, OUT_BUF_SIZE - 1 - strlen(ret));
        strncat(ret, ":", OUT_BUF_SIZE - 1 - strlen(ret));
        strncat(ret, stu_list->course->code,
                OUT_BUF_SIZE - 1 - strlen(ret));
        strncat(ret, "\r\n", OUT_BUF_SIZE - 1- strlen(ret));
    }
